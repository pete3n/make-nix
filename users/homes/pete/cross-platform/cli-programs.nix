...
let
  ...
  shellIntegration = {
    enableBashIntegration = lib.mknix.isLinux make_opts.system;
    enableZshIntegration = lib.mknix.isDarwin make_opts.system;
  };
in
{
	imports = [
  ...
  programs = {
    ...
    wallpaper-scripts = {
      enable = true;
      os = if lib.mknix.isLinux make_opts.system then "linux" else "darwin";
    };
    # Better cat
    bat = {
      enable = true;
