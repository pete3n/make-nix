...
write_line "${BLUE}Target option flags${RESET} (These are boolean, assigning ${BOLD}any truthy${RESET} value will enable them):"
write_line "truthy values are 1 yes Yes YES true True TRUE on On ON y Y"
write_line "Install flags:"
...
write_line "  ${GREEN}- Build and activate the home-manager configuration for the current user using a standalone home-manager configuration; \
autodetect hostname and system type:${RESET}"
...
write_line "  ${GREEN}- Build and activate the standalone home-manager configuration for user sam on host xps-15, set the tags 'debian' and 'server', \
and build for an aarch64-linux platform:${RESET}"
...
write_line "  ${GREEN}- Rebuild and switch the system configuration for host workstation1; autodetect platform; build specialisation \
configurations for wayland and x11_egpu; set default boot menu selection to wayland:${RESET}"
