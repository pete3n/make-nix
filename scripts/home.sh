...
# home can one of multiple targets, if it is the last target, then we
# want to cleanup the environment artifacts, otherwise we need to preserve them.
CURRENT_TARGET="${CURRENT_TARGET:-home}"
others=$(
  printf '%s\n' "${MAKE_GOALS:-}" \
  | tr ' ' '\n' \
...
# traps: only cleanup on EXIT if this is the last goal
trap '
